...
- что если в display уже показан объект работы (слово, выражение, фрагмент), НЕТ пустых ритуальных шагов «Прочитай условие», «Прочитай задание», «Посмотри на слово», «Найди слово в задании», «Посмотри на пример» — объект уже на экране; сразу применяй правило к этому объекту;
...
- если ruleExample уже заполнен, не оставляй knowledgeAid.type="examples" — это дубль; верни knowledgeAid=null либо замени на таблицу/формулу/схему только при реальной необходимости;
...
- rule.kind обязательно укажи по типу знания: rule, formula, table, scheme, list, definition или principle — по нему интерфейс выберет действие родителя («Прочитайте правило вместе», «Посмотрите формулу вместе» и т.д.).
...
- Для безударной гласной в корне ruleExample.display обязательно в формате «пропуск → проверочное слово с ударением на проверяемой гласной → итог». Правильно: «в_да → во́ды → вода́», «тр_ва → тра́вы → трава́». Запрещено: «тр_ва → трава́», «в_да → вода́» — итоговая форма с ударением не на проверяемой гласной НЕ является проверочным словом.
- ruleExample.kind укажи как demo (как это работает), example (рассмотрите пример), compare (сравните два случая) или scheme (посмотрите на схему).
...
- Грубый запрет на пустые ритуалы: если в guidedSteps.display уже показан объект работы, НЕ создавай шаги «Прочитай условие», «Прочитай задание», «Прочитай инструкцию», «Посмотри на слово», «Посмотри на пример», «Найди слово в задании», «Начинаем» без действия. Первый guidedStep обязан сразу спрашивать про применение правила к этому объекту.
...
- Каждый guidedStep обязан продвигать решение. Запрещены пустые ритуалы, если объект уже в display: «прочитай условие», «прочитай задание», «найди пропуск», «скажи, где пропущена буква», «найди слово после пропуска», «посмотри на пример», «посмотри на слово». Даже одна черта внутри слова (например, «д_ржать») уже явно показывает место пропуска.
...
- В заданиях на проверяемую безударную гласную всегда пиши цепочку «пропуск → проверочное слово → итог»: сначала форма с пропуском/безударной гласной, затем однокоренное слово, где ЭТА ЖЕ гласная под ударением, затем итоговая запись. Правильно: «в_да → во́ды → вода́». Неправильно: «тр_ва → трава́» (ударение не на проверяемой гласной). Отмечай ударение явно.
