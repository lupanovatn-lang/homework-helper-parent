...
export async function POST(request: Request) {
  ...
  try {
    ...
    const instruction = mode === "check"
      ...
      : `Распознай ВСЕ отдельные задания на фото или в тексте и создай для каждого адаптивный сценарий помощи родителю. Верни JSON только такого вида: {"tasks":[{"title":"полное короткое название","shortTitle":"2–6 слов для списка","instruction":"как родителю простыми словами объяснить ребёнку, что требуется","simplerInstruction":"ещё более простая версия инструкции","comprehensionQuestion":"вопрос, проверяющий понимание инструкции","rule":{"title":"главное правило в 3–8 словах","text":"одно простое объяснение"},"methodSteps":[{"title":"короткое действие","text":"необязательное пояснение"}],"guidedTitle":"какой реальный элемент задания делаем вместе","guidedSteps":[{"title":"название смыслового шага","prompt":"конкретный вопрос или действие для ребёнка","display":"необязательный объект работы: слово, выражение, фрагмент","options":["2–4 коротких варианта, только если естественны"],"correctOption":"точная строка одного варианта","hint":"одна мягкая подсказка без полного ответа","success":"коротко почему ответ верный и что поняли"}],"independentInstruction":"что ребёнок теперь делает самостоятельно по тому же способу","checkPrompt":"как родителю проверить готовую работу"}]}.
