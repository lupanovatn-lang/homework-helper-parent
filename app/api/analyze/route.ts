...
- поле instruction — дословная формулировка требования из исходного задания. Не перефразируй, не упрощай и не добавляй в неё обращения к ребёнку;
...
- instruction — точная транскрипция самой инструкции с фотографии или из введённого текста. Сохрани исходные слова, порядок действий и формулировки дословно; не перефразируй, не упрощай, не добавляй «тебе нужно» и не включай примеры/слова/решения под инструкцией. Если часть инструкции не читается, не выдумывай её.
...
- rule.title называет конкретное правило, нужное именно для этого задания. rule.text — не больше 2–3 коротких предложений на языке ребёнка соответствующего класса; без повторения инструкции и без подробного алгоритма.
- Всегда добавляй один ruleExample. Он должен наглядно показать всю учебную логику от затруднения через проверку к результату. Для пропущенной буквы формат похож на «з_ма → зи́мы → зима», а explanation объясняет, что именно стало слышно или понятно.
- Не используй в ruleExample ни один элемент текущего задания, если пример раскроет готовый ответ. Подбери близкий по правилу, но другой учебный пример. Не давай несколько однотипных примеров.
...
- knowledgeAid.title должен конкретно называть содержание: «Падежи и вопросы», «Неправильные глаголы», «Формула», «Схема разбора», а не «Полезная памятка». actionLabel — действие вроде «Посмотреть падежные вопросы» или «Открыть таблицу глаголов».
- knowledgeAid.required=true только если без этой опоры выполнить задание невозможно; такую опору интерфейс откроет сразу. Если она лишь облегчает работу, required=false.
