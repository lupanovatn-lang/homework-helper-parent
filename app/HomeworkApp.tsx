...
function StageOne({ task, onNext }: { task: HomeworkTask; onNext: () => void }) {
  const [phase, setPhase] = useState<"check" | "guide" | "retell">("check");
  ...
  return <section className="stage-content"><p className="stage-label">Шаг 1 из 4 · Инструкция</p><h1>Проверьте, понял ли ребёнок, что делать</h1><div className="speech-card"><span>Скажите ребёнку</span><p>«{task.instruction}»</p></div>{phase === "check" && <><div className="question-card instruction-check"><Question size={23} /><div><span>Проверка понимания</span><strong>Попросите ребёнка объяснить задание своими словами</strong><p>«Расскажи, как ты понял(а) задание. Что здесь нужно сделать?»</p></div></div><button className="primary-button flow-primary" onClick={onNext}>Понятно, что делать <ArrowRight size={20} weight="bold" /></button><button className="secondary-button flow-secondary" onClick={() => setPhase("guide")}>Помочь разобраться</button></>}{phase === "guide" && <><div className="question-card instruction-check guidance-step"><Question size={23} /><div><span>Уточняющий вопрос {guideIndex + 1} из {guidingQuestions.length}</span><strong>{currentQuestion}</strong><p>Задайте этот вопрос, только если ребёнок не назвал это действие.</p></div></div><button className="primary-button flow-primary" onClick={nextQuestion}>{guideIndex < guidingQuestions.length - 1 ? "Следующий вопрос" : "Снова пересказать всё"} <ArrowRight size={20} weight="bold" /></button><button className="secondary-button flow-secondary" onClick={nextQuestion}>Это действие уже назвал</button></>}{phase === "retell" && <><div className="question-card instruction-check retell-step"><Question size={23} /><div><span>Снова — вся инструкция</span><strong>Попросите ребёнка ещё раз объяснить задание целиком</strong><p>«Теперь расскажи всё задание своими словами: что нужно сделать и что должно получиться?»</p></div></div><button className="primary-button flow-primary" onClick={onNext}>Понятно, что делать <ArrowRight size={20} weight="bold" /></button><button className="secondary-button flow-secondary" onClick={() => { setPhase("guide"); setGuideIndex(0); }}>Ещё раз помочь вопросами</button></>}<p className="flow-note"><Lightbulb size={16} /> Дальше напомним правило и способ действия</p></section>;
}
